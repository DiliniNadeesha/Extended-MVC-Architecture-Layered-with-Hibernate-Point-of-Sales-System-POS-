...
public class MainFormController implements Initializable {
    ...
    public void btnRestore_OnAction(ActionEvent actionEvent) {
        ...
        if (file != null){
            ...
            // We have to restore the backup file...!
            ...
            if (DBConnection.password.length() > 0){
                ...
                commands = new String[]{"mysql","-h",DBConnection.host,"-u",DBConnection.username,"--port",DBConnection.port,
                        "-p"+ DBConnection.password,DBConnection.db,"-e","source "+file.getAbsolutePath()};
            ...
            task.setOnSucceeded(event -> {
                ...
                new Alert(Alert.AlertType.INFORMATION,"Restore process has been successfill").show();
            });

            task.setOnFailed(event -> {
                this.pgb.setVisible(false);
                this.root.getScene().setCursor(Cursor.DEFAULT);
                new Alert(Alert.AlertType.INFORMATION,"Failed to restore the backup, Contact DEPPO").show();
            });
